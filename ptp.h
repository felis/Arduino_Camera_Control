...
protected:
	...
	uint8_t Command(uint16_t opcode, OperFlags *flags, uint32_t *params);
	uint8_t Transmission(uint16_t opcode, OperFlags *flags, void *pVoid);
	uint8_t Reception(OperFlags *flags, void *pVoid);
	uint8_t Response(OperFlags *flags, uint32_t *params, uint16_t &response);
