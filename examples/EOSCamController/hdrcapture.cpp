...
QState HDRCapture::Active(HDRCapture *me, QEvent const *e) 
{
    ...
    case Q_ENTRY_SIG:
        ...
        return Q_HANDLED();
    case Q_EXIT_SIG:
        Notify(PSTR("::Q_EXIT_SIG\r\n"));
        return Q_HANDLED();
    case Q_INIT_SIG:
        Notify(PSTR("Q_INIT_SIG\r\n"));
        return Q_TRAN(&HDRCapture::SaveSettings);
    case RUN_SIG:
        Notify(PSTR("::RUN_SIG\r\n"));
        return Q_TRAN(&HDRCapture::SaveSettings);
    case ABORT_SIG:
        Notify(PSTR("ABORT_SIG\r\n"));
        return Q_TRAN(&HDRCapture::RestoreSettings);
    case PTP_RC_SIG:
        Notify(PSTR("::PTP_RC_SIG\r\n"));
    ...
    case SET_TIMEOUT_SIG:
        Notify(PSTR("::SET_TIMEOUT_SIG\r\n"));
        return Q_HANDLED();
    case TIMEOUT_SIG:
        Notify(PSTR("::TIMEOUT_SIG\r\n"));
        return Q_TRAN(me->activeHistory);
    case PTP_RC_OK_SIG: 
        Notify(PSTR("::PTP_RC_OK_SIG\r\n"));
        return Q_TRAN(me->activeHistory);
    case TICK_MILLIS_SIG: 
        Notify(PSTR("::TICK_MILLIS_SIG\r\n"));
        return Q_HANDLED();
    }
    return Q_SUPER(&QHsm::top);
}
...
QState HDRCapture::SaveSettings(HDRCapture *me, QEvent const *e) 
{
    ...
    {
    case Q_ENTRY_SIG:
        Notify(PSTR("SaveSettings\r\n"));
        me->activeHistory = (QStateHandler)&SaveSettings;
//        me->rcEvt.rc = me->Eos.GetProperty(EOS_DPC_ExposureCorrection, me->bktOldVal);
...
QState HDRCapture::RestoreSettings(HDRCapture *me, QEvent const *e) 
{
    ...
    {
    case Q_ENTRY_SIG:
        Notify(PSTR("RestoreSettings\r\n"));
        me->activeHistory = (QStateHandler)&RestoreSettings;
        me->rcEvt.rc = me->Eos.SetProperty(EOS_DPC_ExposureCorrection, me->bktOldVal);
        me->theQueue.Push(&me->rcEvt);
        return Q_HANDLED();
    case PTP_RC_OK_SIG:
        return Q_TRAN(&HDRCapture::Inactive);
...
QState HDRCapture::ExpCompSet(HDRCapture *me, QEvent const *e) 
{
    ...
    {
    case Q_ENTRY_SIG:
        Notify(PSTR("ExpCompSet\r\n"));
        me->activeHistory = (QStateHandler)&ExpCompSet; 
        me->rcEvt.rc = me->Eos.SetProperty(EOS_DPC_ExposureCorrection, EXP_COMP_VALUE(me->bktPos));
//        me->rcEvt.rc = PTP_RC_OK;
        me->theQueue.Push(&me->rcEvt);
        return Q_HANDLED();
    case PTP_RC_OK_SIG:
